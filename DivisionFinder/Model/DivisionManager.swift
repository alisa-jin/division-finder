...
struct DivisionManager {
    ...
    // return a list of strings that contain the selected events divisions
    
    func findDivision(divisionName: String) -> [Division] {
        return divisions[divisionName]!.filter { division in
    ...
    func getAllDivisions() -> [String] {
        ...
        var eventDivisions = [String]()
        for event in selectedEvents {
            let div = findDivision(divisionName: event)[0]
            eventDivisions.append("\(div.name) \(event)")
        }
